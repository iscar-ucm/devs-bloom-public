...
clear all
file1='B2S1DetBloom20210802.xlsx'
file2='B2S1LatLon20210802.xlsx'
file3='Atazar1.png'
...
Lon=PosY;
Depth=PosZ;
PosBloom=[150,-100];        %Posición del Bloom
DepthBloom=-5
SizeBloom=50
for i=1:size(Pos,1)
...
subplot(2,1,1),plot(Posicion.PosX,Posicion.PosY,'-',PosX(b),PosY(b),'o');
title('Plan Path');xlabel('X(m)');ylabel('Y(m)');legend('Path','Bloom Detection')
subplot(2,1,2),plot(Posicion.PosX,Posicion.PosZ,'-',PosX(b),PosZ(b),'o');
title('Cross Path');xlabel('X(m)');ylabel('Z(m)');legend('Path','Bloom Detection')


figure(2);
b=boolean(DetBb);
stem3(PosX(b),PosY(b),PosZ(b))
axis([min(PosX) max(PosX) min(PosY) max(PosY) min(PosZ) 0])
grid on
xlabel('X (m)')
ylabel('Y (m)')
zlabel('Z (m)')
