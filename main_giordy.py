"""Módulo creado para realizar pruebas IOT."""
...
class ModelIoT(Coupled):
    ...
    def __init__(self, name: str, commands_path: str, simbody: SimBody5, log_Time=False, log_Data=False):
        ...
        self.add_coupling(fog.get_out_port("o_" + sensor_n.name), cloud.get_in_port("i_" + sensor_n.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_o.name), cloud.get_in_port("i_" + sensor_o.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_a.name), cloud.get_in_port("i_" + sensor_a.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_t.name), cloud.get_in_port("i_" + sensor_t.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_u.name), cloud.get_in_port("i_" + sensor_u.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_v.name), cloud.get_in_port("i_" + sensor_v.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_s.name), cloud.get_in_port("i_" + sensor_s.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_x.name), cloud.get_in_port("i_" + sensor_x.name))
        self.add_coupling(fog.get_out_port("o_" + sensor_y.name), cloud.get_in_port("i_" + sensor_y.name))
        #self.add_coupling(cloud.get_out_port("o_" + fog.name), fog.i_cloud)
